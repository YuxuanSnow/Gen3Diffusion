...
def main():
    ...
    parser.add_argument('--boader_ratio', type=float, help='Ratio of the image border to crop.')
    args = parser.parse_args()

    # Set device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    if 'avatar' in args.checkpoints:
        boader_ratio = 0.227
    else:
        boader_ratio = 0.45

    if args.boader_ratio:
        boader_ratio = args.boader_ratio
    ...
    test_dataset = joint_diffusion_inference_dataset(opt, context_image_path, boader_ratio=boader_ratio, white_bg=True)
