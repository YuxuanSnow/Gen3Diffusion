...
class LGM_timeimagecond_noise_gof(nn.Module):
    def __init__(
        self,
        opt: Options,
    ):
    ...
    def forward(self, data, timestep, step_ratio=1):
        ...
        if self.opt.lambda_normal > 0:
            ...
            render_normal_world = torch.einsum('bvij, bvjn -> bvin', c2w[:, :, :3, :3], render_normal_cam.reshape(B, V, 3, -1)).reshape(B, V, 3, self.opt.output_size, self.opt.output_size)
            render_normal_world = torch.nn.functional.normalize(render_normal_world, p=2, dim=2)

            results['depth_normal_world'] = depth_normal_world
            results['render_normal_world'] = render_normal_world

            normal_error_world = gt_masks * (1 - (depth_normal_world * render_normal_world).sum(dim=2).unsqueeze(dim=2))
            depth_normal_loss = normal_error_world.mean()
            results['loss_normal'] = depth_normal_loss
